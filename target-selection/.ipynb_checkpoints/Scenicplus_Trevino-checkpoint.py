...
# 
# # Greenleaf Lab BrainChromatin Seacelled Multiome 
# 
...
import os
import scipy
work_dir = os.path.expanduser('/wynton/group/pollen/jding/brainchromatin/multiome/seacells/atac_meta_allfrag/')
fragments_dict = {'seacells': os.path.join(work_dir+'counts.tsv')}
...
fragment_matrix = pd.read_csv(fragments_dict['seacells'], sep="\t", header=0,index_col=0)
cistopic_obj = create_cistopic_object(fragment_matrix=fragment_matrix)
print(cistopic_obj)



#cistopic_obj_list=[create_cistopic_object_from_matrix_file(fragment_matrix_file=fragments_dict[key], project=key) for key in fragments_dict.keys()]
...
import pandas as pd
obs = pd.read_csv(os.path.join(work_dir,'obs.csv'),index_col=0)
cistopic_obj.add_cell_data(obs[['celltype']])
cistopic_obj.add_cell_data(obs[['celltype_purity']])


# In[9]:


cistopic_obj.cell_data['celltype'].value_counts()


# In[10]:


# Remove lowe number cell types
cells = cistopic_obj.cell_data[cistopic_obj.cell_data.celltype.isin(
    ['EN1', 'EN3', 'EN2', 'RG', 'IPC', 'EN4', 'EN6','IN1','IN2','IN3'])].index.tolist()
# Subset cisTopic object
cistopic_obj = cistopic_obj.subset(cells, copy=True, split_pattern='-')
print(cistopic_obj)
...
adata = sc.read_h5ad(os.path.join(work_dir, 'seacells/RNAmeta.h5ad'))
work_dir = '/wynton/group/pollen/jding/brainchromatin/multiome/seacells/atac_meta_allfrag/'
...
#adata.obs.index = adata.obs['cellname'].astype(str) +'___seacells'
adata.obs.index = adata.obs['cellname'].astype(str) +'___cisTopic'
...
#have to run the first part on notebooks since internet is necessary
