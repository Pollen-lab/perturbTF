...
# In[61]:
adata1 = sc.read('/wynton/group/pollen/jding/brainchromatin/HM2D/Cellbender/HM2D.h5ad')
adata1.obs['timepoint'] = 'D7'
adata2 = sc.read('/wynton/group/pollen/jding/brainchromatin/HM2D/Cellbender/HM2D_2nd.h5ad')
adata2.obs['timepoint'] = 'D7'
adata3 = sc.read('/wynton/group/pollen/jding/brainchromatin/HM2D/Cellbender/HM2D_3rd.h5ad')
adata3.obs['timepoint'] = 'D7'
adata_query=anndata.AnnData.concatenate(adata1,adata2,adata3, join='outer',batch_categories=['HMD7','HM2ndD7','HM3rdD7']) 
...
sc.pl.umap(adata_query, color=['timepoint','batch_name','individual','Ident'],frameon=False,ncols=1,save='scvi-query')
...
directory='/wynton/group/pollen/jding/brainchromatin/Li/'
dir_path_scan = os.path.join(directory,'scanvi_HM2D_model/')
...
sc.pl.umap(adata_query,color=["predictions",'leiden-.3','individual','Ident'],frameon=False,ncols=1,save='scanvi-query-vivo')
...
sc.pl.umap(adata_query,color=viewgenes,use_raw=False,save='genes-vivo')

'''
import pandas as pd
adata_query.obs.to_csv(os.path.join(directory,'query_HM2D.obs.csv'))
df = pd.DataFrame(adata_query.obsm['X_umap'], columns=['UMAP1', 'UMAP2'])
df['cell_id'] = adata_query.obs.index
df.to_csv(os.path.join(directory,'query_HM2D.X_umap.csv'))
df = pd.DataFrame(adata_query.obsm['X_scANVI'])
df['cell_id'] = adata_query.obs.index
df.to_csv(os.path.join(directory,'query_HM2D.X_scANVI.csv'))
